...
func NewPeerConnectionState(c *Client) (*PeerConnectionState, error) {
	...
	c.Hub.Lock()
	if _, ok := c.Hub.TrackChannels[c.Channel]; !ok {
		c.Hub.TrackChannels[c.Channel] = make(map[string]*webrtc.TrackLocalStaticRTP)
	}
	c.Hub.PeerChannels[c.Channel] = append(c.Hub.PeerChannels[c.Channel], PeerConnectionState{peerConnection, c})
	c.Hub.Unlock()
	...
	peerConnection.OnICECandidate(func(i *webrtc.ICECandidate) {
		...
		c.Lock()
		if writeErr := c.Conn.WriteJSON(&WebsocketMessage{
			Event:   "candidate",
	...
	peerConnection.OnConnectionStateChange(func(p webrtc.PeerConnectionState) {
		...
		case webrtc.PeerConnectionStateClosed:
			c.Hub.signalPeerConnections(c.Channel)
		default:
		}
	})
	...
	peerConnection.OnTrack(func(t *webrtc.TrackRemote, _ *webrtc.RTPReceiver) {
		...
		trackLocal := c.Hub.addTrack(c.Channel, t)

		defer c.Hub.removeTrack(c.Channel, trackLocal)
...
func (h *Hub) signalPeerConnections(channel string) {
	...
	attemptSync := func() (tryAgain bool) {
		for i := range h.PeerChannels[channel] {
			...
			if err = h.PeerChannels[channel][i].client.WriteJSON(&WebsocketMessage{
				Event:   "offer",
